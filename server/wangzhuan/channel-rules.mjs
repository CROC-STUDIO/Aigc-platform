...
export async function loadChannelRuleStore(context) {
  ...
  const store = await readJsonOrDefault(paths.channelRulesPath, DEFAULT_RULE_STORE);
...
export async function getChannelRules(context, query = {}) {
  ...
  const paths = wangzhuanPaths(context);
  const existedStore = await readJsonOrDefault(paths.channelRulesPath, null);
  const store = existedStore ?? DEFAULT_RULE_STORE;
  if (!existedStore) await writeAtomicJson(paths.channelRulesPath, store);

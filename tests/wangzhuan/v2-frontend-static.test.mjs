...
test("wangzhuan v2 page uses required native controls and field labels", async () => {
  ...
  assert.match(html, /id="wzProductLink" type="url"/);
  assert.match(html, /产品链接/);
  ...
  assert.match(html, /id="wzCodexPromptTestBtn"[\s\S]*测试生成 Seedance Prompt/);
...
test("wangzhuan v2 script submits array-compatible selects and uses background jobs", async () => {
  ...
  assert.match(js, /els\.decompositionStatus\.hidden = true/);
  assert.doesNotMatch(js, /场景切点、抽帧、LLM 秒级剧情拆解/);
  assert.match(js, /\/auto-seedance-prompt-jobs/);
  assert.match(js, /\/api\/wangzhuan\/product-info/);
  assert.match(js, /function applySelectedProductLibrary/);
  assert.match(js, /function renderProductLibraryDetail/);
  assert.match(js, /productInfoId/);
  assert.match(js, /coreSellingPoints/);
  assert.match(js, /applyProductLibraryAssets/);
  assert.match(js, /function nextAssetEntryKey/);
  assert.match(js, /function pendingAppendFiles/);
  assert.match(js, /function validateAssetAppendLimit/);
  assert.match(js, /function validateImageOnlyEndingAsset/);
  assert.match(js, /CTA 图.*只能上传图片/);
  assert.match(js, /Ending 图.*只能上传图片/);
  assert.match(js, /validateImageOnlyEndingAsset\(input, assetKey\) && validateAssetAppendLimit\(input, assetKey\)/);
  assert.match(js, /function removeBranchAssetEntry/);
  assert.match(js, /function commitSelectedAssetFiles/);
  assert.match(js, /pendingAssetFiles/);
  assert.match(js, /data-remove-asset-key/);
  assert.match(js, /delete branch\.assetFileNames\[entryKey\]/);
  assert.match(js, /input\?\.closest\("\.wz-asset-slot"\)\?\.addEventListener\("click"/);
  assert.match(css, /\.wz-v2-body \.wz-asset-slot \.wz-v2-asset-remove/);
  assert.match(js, /已记录，待上传/);
  assert.match(js, /commitSelectedAssetFiles\(event\.target, assetKey\)/);
  ...
  assert.match(js, /function renderCodexPromptTestResult\(\)/);
  assert.match(js, /state\.codexPromptTestResult/);
  assert.match(js, /function startCodexPromptTestJob\(\)/);
  assert.match(js, /function pollCodexPromptTestJob\(batchId, jobId\)/);
  assert.match(js, /测试版 Seedance prompt 任务已提交/);
  assert.match(js, /测试版 Seedance prompt 完成/);
  assert.match(js, /els\.codexPromptTestBtn/);
  assert.match(js, /value\(els\.codexTestStoreLink\) \|\| value\(els\.productLink\)/);
  ...
  assert.match(js, /function decompositionMaxRetries\(model = selectedDecompositionModel\(\)\) \{\s*return isGeminiDecompositionModel\(model\) \? 3 : 2;\s*\}/);
  assert.doesNotMatch(js, /isGeminiDecompositionModel\(model\) \? 3 : 0/);
...
test("v2 captures wangzhuan output template fields for Seedance planning", async () => {
  ...
  assert.doesNotMatch(html, /id="wzOutputTemplateMode"/);
  assert.doesNotMatch(html, /出量模板/);
  assert.doesNotMatch(html, /由视频拆解决定/);
  assert.doesNotMatch(html, /按参考视频的核心转化基调、段落风格和转化信号自动生成/);
  assert.doesNotMatch(html, /<option value="three_slice_net_earning"/);
  assert.doesNotMatch(html, /<option value="short_drama_earning_highlight"/);
  assert.doesNotMatch(html, /id="wzSliceStrategy"/);
  assert.doesNotMatch(html, /切片策略/);
  assert.doesNotMatch(html, /按视频拆解自动切片/);
  assert.doesNotMatch(html, /先按原视频剧情段落切片/);
  assert.doesNotMatch(html, /<option value="fixed_15s"/);
  ...
  assert.doesNotMatch(html, /id="wzMoneyVisuals"/);
  assert.doesNotMatch(html, /网赚视觉元素/);
  assert.match(html, /id="wzSubtitleWorkflow"/);

  assert.match(js, /const AUTO_OUTPUT_TEMPLATE_MODE = "reference_fission"/);
  assert.match(js, /const AUTO_SLICE_STRATEGY = "auto_10_15s_multi_slice"/);
  assert.match(js, /const DEFAULT_MONEY_VISUALS = \["cash", "coin_burst", "cash_rain", "reward_number_growth", "withdrawal_success", "arrival_animation", "withdrawal_record"\]/);
  assert.match(js, /outputTemplateMode:\s*AUTO_OUTPUT_TEMPLATE_MODE/);
  assert.match(js, /sliceStrategy:\s*AUTO_SLICE_STRATEGY/);
  assert.match(js, /targetSegmentCount:\s*targetSegmentCountValue\(\)/);
  assert.match(js, /durationSec:\s*compatibleDurationSecValue\(\)/);
  assert.doesNotMatch(js, /durationSec:\s*Number\(value\(els\.duration\) \|\| 15\)/);
  assert.doesNotMatch(js, /outputTemplateMode:\s*value\(els\.outputTemplateMode\)/);
  assert.doesNotMatch(js, /sliceStrategy:\s*value\(els\.sliceStrategy\)/);
  assert.doesNotMatch(js, /moneyVisuals:\s*selectedValues\(els\.moneyVisuals\)/);
  assert.match(js, /subtitleWorkflow:\s*value\(els\.subtitleWorkflow\)/);
  assert.match(js, /const signatureFields = \[[\s\S]*"outputTemplateMode"[\s\S]*"sliceStrategy"[\s\S]*"targetSegmentCount"[\s\S]*"moneyVisuals"[\s\S]*"subtitleWorkflow"[\s\S]*"branches"[\s\S]*\]/);
  assert.match(js, /function currentDraft\(\)[\s\S]*const active = branches\[state\.activeBranchIndex\] \|\| primary/);
  assert.match(js, /function currentDraft\(\)[\s\S]*outputTemplateMode:\s*AUTO_OUTPUT_TEMPLATE_MODE/);
  assert.match(js, /function currentDraft\(\)[\s\S]*sliceStrategy:\s*AUTO_SLICE_STRATEGY/);
  assert.match(js, /function currentDraft\(\)[\s\S]*moneyVisuals:\s*active\.moneyVisuals \|\| DEFAULT_MONEY_VISUALS/);
  ...
  assert.match(js, /function planSignatureInput\(\)[\s\S]*outputTemplateMode:\s*AUTO_OUTPUT_TEMPLATE_MODE/);
  assert.match(js, /function planSignatureInput\(\)[\s\S]*sliceStrategy:\s*AUTO_SLICE_STRATEGY/);
  assert.match(js, /function planSignatureInput\(\)[\s\S]*moneyVisuals:\s*active\.moneyVisuals \|\| DEFAULT_MONEY_VISUALS/);
  ...
  assert.match(js, /function estimateRequest\(\)[\s\S]*outputTemplateMode:\s*AUTO_OUTPUT_TEMPLATE_MODE/);
  assert.match(js, /function estimateRequest\(\)[\s\S]*sliceStrategy:\s*AUTO_SLICE_STRATEGY/);
  assert.match(js, /function estimateRequest\(\)[\s\S]*moneyVisuals:\s*active\.moneyVisuals \|\| DEFAULT_MONEY_VISUALS/);
  ...
  assert.match(js, /function addBranch\(\)[\s\S]*outputTemplateMode:\s*AUTO_OUTPUT_TEMPLATE_MODE/);
  assert.match(js, /function addBranch\(\)[\s\S]*sliceStrategy:\s*AUTO_SLICE_STRATEGY/);
  ...
  assert.match(jobs, /const PLAN_SIGNATURE_FIELDS = Object\.freeze\(\[[\s\S]*"outputTemplateMode"[\s\S]*"sliceStrategy"[\s\S]*"targetSegmentCount"[\s\S]*"moneyVisuals"[\s\S]*"subtitleWorkflow"[\s\S]*"branches"[\s\S]*\]\)/);
});
...
test("v2 plan editor exposes output-template plan fields", async () => {
  ...
  assert.doesNotMatch(js, /data-plan-field="moneyVisuals"/);
...
test("v2 preview exposes fission and repair fields", async () => {
  const js = await readFile(new URL("../../public/wangzhuan-v2.js", import.meta.url), "utf8");
  assert.match(js, /data-plan-field="conversionEffectOpportunities"/);
  assert.match(js, /data-plan-field="storySegmentIndex"/);
  assert.match(js, /data-plan-field="seedanceSliceIndex"/);
  assert.doesNotMatch(js, /data-plan-field="mandatoryMoneyVisualCarrier"/);
  assert.match(js, /readList\("conversionEffectOpportunities"\)|readJsonList\("conversionEffectOpportunities"\)/);
});

test("plan job signature includes fission repair fields", async () => {
  const jobs = await readFile(new URL("../../server/wangzhuan/background-jobs.mjs", import.meta.url), "utf8");
  assert.match(jobs, /"conversionEffectOpportunities"/);
  assert.match(jobs, /"storySegments"/);
  assert.match(jobs, /"seedanceSlices"/);
  assert.match(jobs, /"currencySymbol"/);
});

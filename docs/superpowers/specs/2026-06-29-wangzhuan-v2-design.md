...
- v2 作为一个完整工作流发布范围：AI 拆解后台化和 Seedance 预案后台化都完成前，v2 不算可验收。
...
v2 的产品目标是完整减少两个核心等待点：AI 拆解视频、Seedance 预案生成。实现可以分批提交，但发布验收不能拆成“只有拆解后台化”的半成品；两个后台任务能力都是 v2 首版完整工作流的一部分。
...
- 强影响字段变化必须让预案 stale：产品名、产品链接、产品素材引用、目标渠道、目标地区、语言、素材方向、其他素材方向、口播风格、承诺等级、币种、CTA、Ending、脚本裂变规则补充、补充提示词、限定提示词。
- 后处理字段变化不应让 Seedance 预案 stale：免责声明贴片位置、字号、贴片高度、底部距离、左右距离。这些字段只影响最终视频后处理贴片。
- 免责声明文案是否让预案 stale 取决于当前实现是否把它写入 Seedance prompt。当前合同是“不写入 Seedance prompt 或 payload”，因此首版按后处理字段处理，不触发预案 stale。
...
- `id`
- `type`: `decomposition` / `seedance_plan` / `asset_review` / `generation` / `qc`
...
- AI 拆解后台化和 Seedance 预案后台化都完成前，v2 不能作为完整入口上线。
...
以下是实现批次，不是产品阶段。每一批可以独立开发和测试，但 v2 的可验收范围仍是完整工作流。
...
- 批次 1-5 都完成后，v2 才算首版完整工作流。
- 批次 3 和批次 4 都完成前，不能把 v2 作为默认生产入口。
- 任何中间批次可以合入代码，但入口应保持显式 v2 或内部验证入口，不替换旧版。

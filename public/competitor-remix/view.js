...
function formTemplate(capabilityId, modeId, draft) {
  if (capabilityId === "remove" && modeId === "seedance") {
    return `<div class="remix-form">${advanced(`<label class="remix-field remix-field-wide"><span>处理提示</span><textarea data-field="prompt" rows="4">${escapeHtml(draft.prompt)}</textarea></label>${selectField("画面比例", "ratio", draft.ratio, [["auto", "自动"], ["9:16", "9:16"], ["16:9", "16:9"], ["1:1", "1:1"]])}${selectField("分辨率", "resolution", draft.resolution, [["720p", "720p"], ["1080p", "1080p"]])}${field("切片秒数", "segmentSeconds", draft.segmentSeconds, { type: "number", min: 1, max: 30, step: 1 })}${priorityField(draft)}`)}</div>`;
  }
  ...
  if (capabilityId === "ending") {
    return `<div class="remix-form">${advanced(`${field("检测尾段秒数", "tailDetectSeconds", draft.tailDetectSeconds, { type: "number", min: 1, max: 30, step: 1 })}${field("复核阈值", "reviewThreshold", draft.reviewThreshold, { type: "number", min: 0, max: 1, step: 0.05 })}${selectField("裁剪方式", "trimMode", draft.trimMode, [["fast", "快速"], ["precise", "精确"]])}${field("安全边距（毫秒）", "safeTrimMarginMs", draft.safeTrimMarginMs, { type: "number", min: 0, max: 5000, step: 1 })}<label class="remix-field remix-field-wide"><span>竞品或导流关键词（每行一个）</span><textarea data-field="competitorKeywords" rows="4">${escapeHtml(draft.competitorKeywords)}</textarea></label><label class="remix-check"><input data-field="allowReencode" type="checkbox"${draft.allowReencode ? " checked" : ""} />允许必要时重新编码</label>${priorityField(draft)}`)}</div>`;
  }
  if (capabilityId === "language" && modeId === "subtitle_translate") {
    return `<div class="remix-form"><div class="remix-form-grid">${field("目标语言", "targetLanguage", draft.targetLanguage)}</div>${advanced(`${selectField("字幕来源", "sourceMode", draft.sourceMode, [["auto", "自动识别"], ["embedded", "内嵌字幕"], ["burned", "画面字幕"]])}${selectField("回写方式", "renderMode", draft.renderMode, [["subtitle_band", "字幕带"], ["overlay", "画面覆盖"]])}${selectField("字幕区域", "subtitleRoiMode", draft.subtitleRoiMode, [["auto", "自动"], ["full", "全画面"]])}${selectField("原字幕处理", "subtitleRemovalMode", draft.subtitleRemovalMode, [["band", "字幕带移除"], ["none", "不移除"]])}${priorityField(draft)}`)}</div>`;
  }
  if (capabilityId === "language" && modeId === "rewrite") {
    return `<div class="remix-form"><div class="remix-form-grid">${field("目标语言", "targetLanguage", draft.targetLanguage)}</div>${advanced(priorityField(draft))}</div>`;
  }
  if (capabilityId === "analysis") {
    return `<div class="remix-form">${advanced(`<label class="remix-check"><input data-field="useLlm" type="checkbox"${draft.useLlm ? " checked" : ""} />启用模型分析</label>${priorityField(draft)}`)}</div>`;
  }
  return "";
}
